...
param_pem = params.parameter_bytes(serialization.Encoding.PEM, serialization.ParameterFormat.PKCS3)
...
r.set("key_exchange_channel", json.dumps({
...
while True:
    try:
        # Real-life system data
        metric = json.dumps({
            "cpu": psutil.cpu_percent(),
            "mem": psutil.virtual_memory().percent
        })
        packet = secure.encrypt(metric, counter)
